...
```console
decktalk build --no-voice && decktalk check && decktalk verify
```
...
`status`, `align`, `check`, `verify`, and `doctor` share one exit rule. Use their exit codes as the assertions of
a job.
