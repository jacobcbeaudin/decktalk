...
|---|---|---|
| `decktalk build` | an error, `PAGE ERROR`, a cue phrase that is not found, an unknown cue id, an uncued element, `BLACK`, `SPEECH AT CUT` | `TRUNCATED`, `STALLED`, `NO COVER`, `BLACK?`, `SPOKEN SYMBOL?`, every cue result |
| `decktalk record` | `PAGE ERROR`, `STALLED`, `TRUNCATED`, `NO COVER`, `KATEX ERROR`, `KATEX NOT LOADED` | `BLACK?`, unless you pass `--strict` |
