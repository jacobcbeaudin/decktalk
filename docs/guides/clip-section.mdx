...
A `NN-slug.words.json` file from `build/narration` has this format. `decktalk clip` writes the words file of a clip that
...
<Steps>
  ...
  <Step title="Find each clip's times, and write its words file">
    ...
    ```console
    read B0 B1 < <(python3 clip_words.py build/narration/01-open.words.json media/edit-before.words.json)
    read A0 A1 < <(python3 clip_words.py ../my-lesson-after/build/narration/01-open.words.json media/edit-after.words.json)
    ```
  </Step>
  ...
  <Step title="Cut the clips">
    ...
    ffmpeg -ss $B0 -to $B1 -i build/sections/01.mp4 -ss $B0 -to $B1 -i build/narration/01-open.mp3 -loop 1 -framerate 25 -i media/edit-before-bg.png \
      -filter_complex "$F" -map "[v]" -map "[a]" -shortest -r 25 -c:v libx264 -crf 12 -c:a pcm_s16le media/edit-before.mov
    ffmpeg -ss $A0 -to $A1 -i $A -ss $A0 -to $A1 -i ../my-lesson-after/build/narration/01-open.mp3 -loop 1 -framerate 25 -i media/edit-after-bg.png \
