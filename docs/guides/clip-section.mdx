...
<Steps>
  ...
  <Step title="Build the video">
    ...
    {/* sample: decktalk build --no-voice in clip-lesson, the starter with the clip section added and no clip file (assemble lines) */}
    ```text
  ...
  <Step title="Cut the clip into the video">
    ...
    {/* sample: decktalk assemble in clip-lesson after the test clip above is in place */}
    ```text
    [cut ] 03  03.webm (t0 1.44s trimmed) -> 03.mp4  (14.720s)
...
{/* sample: decktalk assemble --strict in clip-lesson, the starter with the clip section added and no clip file */}
```text
section 04: media/intro.mov is missing, so a slate plays for 4s. Drop your clip at that path. The section is optional, so --strict allows the slate
```
...
{/* sample: decktalk status in clip-lesson with no media/intro.mov */}
```text
...
{/* sample: decktalk status in clip-lesson after the steps above, the section rows */}
```text
...
<Steps>
  <Step title="Find the span">
    ...
    {/* sample: decktalk words --only 1 after decktalk build --no-voice on a fresh starter, header and the first rows */}
    ```text
    sections 1, words 26
  ...
  <Step title="Cut the clip">
    ...
    {/* sample: decktalk clip 1 --start 0.48 --end 1.92 --hold 0.5 --out media/open-line.mp4 after decktalk build --no-voice on a fresh starter */}
    ```text
...
<Steps>
  ...
  <Step title="Build the video">
    ...
    {/* sample: the assemble stage of decktalk build --no-voice, the starter with section 0 added as above and no media/open.mp4 (first lines) */}
    ```text
