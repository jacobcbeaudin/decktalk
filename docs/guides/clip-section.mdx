...
The script needs a `## 3. B-roll` heading between sections 2 and 4, and `cues.json` keys
its cues by the new section numbers. A section's `scene` names the scene in the page, so
renumbering the sections leaves the pages as they are.
