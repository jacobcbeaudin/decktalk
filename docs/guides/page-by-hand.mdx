...
This guide requires writing JavaScript. The runtime is one file with no dependencies, and
most pages should include it. A page
built with a framework of its own can implement the contract instead. This guide lists
...
| Adds `&words=word@s,…`, the section's spoken words with their seconds after narration t=0, whenever the timeline has words. | May read it to show text one word at a time, or ignore it. |
...
The page below honors the whole contract in about forty lines. It has one scene with one
