...
After a [build without voice](/concepts/how-it-works#the-build-without-voice), the timeline line ends in
...
The narration cache is `build/narration/takes.json`. It holds one hash per section, made from these values:
...
<Warning>Run `decktalk build --no-voice` only if the last build had no voice. In a voiced project, it stops with an
...
A build without voice after a voiced build would empty the narration cache. The next voiced build would then voice
every section again and spend credits on all of them. So a `narrate` or `build` without voice stops when
`build/narration/takes.json` holds voiced takes, and it writes nothing.
...
1. The take index notes whether its entries came from a build without voice.
2. `narrate` reuses the previous take index only when it is the same kind of build.
3. A build without voice writes a [click track](/concepts/how-it-works#the-build-without-voice) over the voiced mp3 of
   the same name, and it replaces the take index.
