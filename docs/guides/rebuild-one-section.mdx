...
<Warning>Run `decktalk build --silent` only if the last build was silent. In a voiced project, it empties
the narration cache, and the next voiced build spends credits on every section.</Warning>
...
A silent build after a voiced build empties the narration cache. The next voiced build then voices every
...
After a project has been voiced, rehearse with a silent build in a copy of the project. Neither kind of
