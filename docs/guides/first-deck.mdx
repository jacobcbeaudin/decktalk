...
<Steps>
  ...
  <Step title="Add the cues to cues.json">
    ...
    The `4.1` cue on `$start` only mounts the slide at 0.0 s, so `decktalk verify` skips it as `REFERENCE_CLAMPED`. If the speech is shorter than `min_seconds`, `decktalk align` reports an uncertain finding. [Cues](/concepts/cues) explains how phrases match.
  </Step>
...
1. Open `deck/index.html?slide=4.1` in a browser. The slide mounts with every element visible.
2. Open `deck/index.html?scene=4&hud=1`. The scene plays in preview, with the clock on top.
...
4. Estimate every reveal with `decktalk preflight`. It freezes the page before and at each cue, and flags a reveal
   too small for `verify` as `THIN CHANGE?` or `NO CHANGE`. The five point moves of this slide each change about
   0.15 percent of the frame, under the 0.3 percent floor, so `preflight` reads them as `THIN CHANGE?`. A larger
   dot clears the floor.
...
sec  speech   need  cues
 04    49.3   30.0  4.1@0.0,4.1bowl@7.39,4.1p0@11.83,4.1p1@18.73,4.1rule@22.18,4.1p2@26.13,4.1p3@27.11,4.1think@33.03,4.1over@39.44,4.1settle@47.53
```
...
4:4.1rule           22.18    66.30    0.43    0.00    -20ms   -29ms  changed
4:4.1over           39.44    83.56    0.71    0.00    -40ms   -44ms  changed
```
