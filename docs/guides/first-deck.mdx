...
page writes one section, the gradient descent scene from the scaffold, and shows every
...
<Steps>
  ...
  <Step title="Build and verify">
    ...
    decktalk verify 3:3.1eq 3:3.1p1 5:5.1a
    ```
    ...
    5:5.1a               2.55   132.43    0.41    0.00    -30ms   -63ms  changed
    ```
    ...
    further from its word than that limit is reported as `OFF CUE`, and the build's exit
    code says so.
...
| `data-sync` on an element | The element reveals whole. | The element reveals one word at a time from the `?words=` list the recorder passes. |
