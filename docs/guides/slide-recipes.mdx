...
`cues.json`, and the scene for `deck/index.html`. Each recipe carries the CSS it needs in a
`<style>` block, because the scaffold's stylesheet defines only its own classes. Paste the
style and the scene together. They are recipes rather
...
An element with `data-sync` appears one word at a time, each at the second the voice
reaches it, so a line on the slide keeps pace with the narration. The scaffold uses it for
the caption in its edit section, which lights up as the voice reads the edited line. The text
...
cue. It needs no `data-fx`. The runtime gives a synced element no container animation,
because a fade on the container would fight the per-word reveal.
...
<style>
  ...
  .stack > * { margin: 0; }
  .line { font: 450 36px/1.4 var(--sans); color: var(--ink); }
  .lower { font: 500 22px/28px var(--mono); letter-spacing: .14em; text-transform: uppercase; color: var(--mute); }
</style>
<script>
...
DeckTalk.scene(5, { name: "Close", steps: [
  { id: "5.1", hold: 10, cues: { "5.1a": 2, "5.1b": 4.5, "5.1c": 7 }, render: () => `
    <div class="slide">
...
the script by one word appears whole, and the recorder logs
`data-sync text not found in the spoken words`. In a browser, `?scene=5` shows each
line whole, because the preview has no word times. To check the line at one cue without
playing the section, open `?step=5.1&cue=5.1b`. Step 5.1 names its cues in `cues`, so the
page fires them up to `5.1b` and leaves `5.1c` hidden.
[The runtime reference](/reference/runtime#data-attributes-inside-a-step) has the rules.

## A derivation that appears line by line
...
```html deck/index.html
<style>
  .stack { position: absolute; left: 120px; top: 300px; display: flex; flex-direction: column; gap: 56px; }
...
the text arrives. The second step is the "after", and its cue `4.2` mounts it.
...
## A code block that appears one line per phrase
...
Two columns reveal left then right, and each has a heading and a caption. The `<style>`
block lays them out with flex, and each column gets one cue.
...
This recipe requires a few lines of JavaScript before the runtime starts. The recorder
starts the narration clock only after `window.__sceneReady` resolves, so a page with a
large image should set that promise to the image's decode. Set it in a
...
<style>
  ...
  .note { position: absolute; left: 260px; top: 960px; margin: 0; }
</style>
...
DeckTalk.scene(7, { name: "The status report", steps: [
  { id: "7.1", hold: 10, cues: { "7.1shot": 0.5, "7.1note": 5 }, render: () => `
    <div class="slide">
      ...
      <p class="caption note" data-cue="7.1note">Captions and chapters sit beside the video.</p>
    </div>` },
]});
</script>
```
...
otherwise have set. [The runtime reference](/reference/runtime#window__sceneready) has
the details.
