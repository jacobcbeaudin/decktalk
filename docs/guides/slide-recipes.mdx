...
An element with `data-text="spoken"` shows its words one at a time, as the voice says them. The scaffold's
...
<script>
DeckTalk.scene(8, { name: "The derivative of x squared", slides: [
  { id: "8.1", hold: 14, preview: { "8.1a": 1, "8.1b": 5, "8.1c": 9 }, render: () => `
    <div class="slide">
...
The recorder starts the clock only after `window.__decktalk.ready` resolves. This recipe adds the
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.__decktalk.ready = Promise.all([window.__decktalk.ready, decoded]).then(([ready]) => ready);
});
</script>
```
</CodeGroup>

The listener runs after the runtime's own `DOMContentLoaded` listener, so `window.__decktalk.ready`
already holds the runtime's Promise. [`window.__decktalk.ready`](/reference/runtime#window-__decktalk-ready)
