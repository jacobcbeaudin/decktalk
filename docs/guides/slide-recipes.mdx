---
title: Slide recipes
description: Five copyable DeckTalk slides with script lines, cues, and HTML, including a KaTeX derivation, a chat before-and-after, a code block, a two-column comparison, and a screenshot slide.
icon: "layer-group"
---
...
`decktalk init` writes, so paste them into the scaffold and build. The scene and step
ids are placeholders, and any section number works as long as the same number appears in
the script heading, the `[[section]]` table, and the `cues.json` key.
...
Each line of the derivation is its own element with its own cue, and `data-display`
typesets it as display math. The answer uses `pop` so that it lands harder than the steps.
