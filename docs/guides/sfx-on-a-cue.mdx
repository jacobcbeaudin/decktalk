...
<Steps>
  ...
  <Step title="Preview the request">
    ...
    ```console
    decktalk soundscape --dry-run --only tick
    ```

    {/* sample: decktalk soundscape --dry-run --only tick, scaffold with [soundscape.sfx.tick] uncommented */}
    ```text
  ...
  <Step title="Mix the effect into the video">
    ...
    {/* sample: decktalk assemble, silent build of the scaffold with the [[mix.sfx]] table above and no tick.mp3 (one line) */}
    ```text
...
- **Swell or mute the underscore on a phrase:** [Markers](/concepts/sound#markers)
