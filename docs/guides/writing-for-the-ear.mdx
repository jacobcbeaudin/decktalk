...
To keep a section's last frame on screen after its narration ends, set `hold_seconds` on that page section. To open a section on silence, set `lead_seconds`. [cues.json](/reference/cues-json) documents `$start` and `$end`.
