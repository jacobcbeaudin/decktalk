...
that names the visual. The cue for "Pick a point, theta, and its height is the loss" names
`Pick a point` and lands on "Pick", a verb whose referent, the ball, appears as it is said.
A phrase such as "and then" lands on nothing in particular. The scaffold's cues are nouns
or verbs with a concrete referent: `weights`, `Pick a point`, `learning rate`,
`Build again`, `Open source`.
...
learning rate, so each ghost ball sets off as its own event.

```md script.md
Too large, and you zigzag. [beat] Larger still, and you fly out. [beat] Too small, and you
crawl.
```

A timed pause earns its place before a question the audience should answer, such as
`[pause 4]` after "what happens if eta is too large?", so the silence leaves room to think.

Anything else in square brackets is a stage direction. It is not spoken and it is a beat
where it sat, so a direction such as `[Deck scene 3. A loss surface fades up.]` is both a note
...
the narrator pointing at something. The scaffold trails `The best eta` by 0.2 seconds, so
the ball sets off on its last moves to the bottom of the bowl just after the sentence starts.
