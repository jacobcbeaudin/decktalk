...
that names the visual. "Keep two weights, theta one and theta two" cues on `theta one` and
lands on "theta", which is a noun the listener can attach to the axis label that fades in.
A phrase such as "and then" lands on nothing in particular. The scaffold's cues are nouns
or verbs with a concrete referent: `many variables`, `Pick a point`, `learning rate`,
`Build again`, `Narrated presentations`.
...
Use beats freely and timed pauses sparingly. The scaffold pauses for four seconds before
...
and each bounce climbs higher, until you fly out.
```
...
---------------------------------------------------------------------
 1  open                      48   0:21    --      0:22  -
 2  four-files                74   0:32    --      0:34  -
 3  gradient-descent         179   1:17    0:20    1:28  -
 4  the-edit                  42   0:18    --      0:21  -
 5  close                     19   0:08    --      0:09  -
---------------------------------------------------------------------
    total                    362   2:35            2:56
```

The `target` column is the budget's length. Section 3 has a budget of twenty seconds,
from 0:40 to 1:00, and the silent rehearsal runs to 1:28. That gap is the signal to
