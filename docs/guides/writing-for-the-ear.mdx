...
that names the visual. "Pick a point, theta, and its height is the loss" cues on
...
the ball sets off on its last steps to the bottom of the bowl just after the sentence starts.
...
The `target` column is the budget's length. Section 3 now has a budget of twenty seconds,
