...
This page lists what DeckTalk needs, what it downloads and when, and which commands spend ElevenLabs credits. Read it before you set up a new machine or a CI runner, and before your first voiced build.
...
- DeckTalk downloads Chromium and ffmpeg itself, the first time a command needs one of them. KaTeX ships inside the wheel, and you install nothing else by hand.
- `decktalk install` makes both downloads up front instead, and on Linux it adds Chromium's system libraries. Nothing requires it: see [What DeckTalk downloads](#what-decktalk-downloads).

The [quickstart](/quickstart) installs DeckTalk with uv, pipx, or pip.

### glibc, not musl

On Linux, DeckTalk needs glibc. Playwright, which records the slides, publishes no musllinux wheels, so on a musl system the install cannot even resolve. Alpine and the other musl distributions are unsupported, and no setting changes that. DeckTalk's install script checks before it installs anything and stops with that reason, rather than leaving you a page of resolver output to read.

Containerise on a Debian-based image: `debian`, `ubuntu`, or `python:3.13-slim`, never `python:3.13-alpine`. Debian is also where `decktalk install` can add Chromium's system libraries, because Playwright installs them with apt-get. [Linux system libraries](#linux-system-libraries) gives the rule for a distribution that has no apt-get.
...
Two downloads, one time per machine, and DeckTalk makes them itself. The first command that needs a browser fetches Chromium, and the first command that needs to encode fetches ffmpeg. Each one prints a line before its download starts, so a long first build says what it is waiting for. `decktalk doctor` then reports on each download without making either.

| Download | How DeckTalk gets it |
|---|---|
| Chromium | It runs `playwright install chromium`, the same command `decktalk install` runs. |
| ffmpeg and ffprobe | It downloads one pinned GPL build, checks each archive against the SHA-256 DeckTalk records for it, and unpacks it into `ffmpeg/<version>-<platform>` in DeckTalk's cache. |

### What `decktalk install` is for

`decktalk install` makes both downloads up front and does nothing else. No command needs it to have been run, and it is worth running when you want the download to happen at a time you choose:

- **A Docker image.** A `RUN decktalk install` layer is cached, so the download is not in every build.
- **A CI job.** The step is one place to cache, and one place to read when a download fails.
- **A machine that will be offline.** Fetch everything while there is a network.
- **Linux.** It is the one command that installs Chromium's system libraries, and therefore the one command that may ask for a password. A build never asks, because a build runs unattended in a script or in CI. If Chromium will not start on Linux, this is the fix.
...
On Linux, `decktalk install` passes `--with-deps` to Playwright, and Playwright installs Chromium's system libraries with apt-get.
...
A build fetches Chromium without `--with-deps`, and therefore without sudo, because an unattended build that stops at a password prompt is a build that hangs. On a Linux machine whose libraries are missing, the browser downloads and then will not start, and the error says to run `decktalk install`.
...
- **`install` never ran.** Nothing is wrong: DeckTalk downloads Chromium the first time it needs a browser and the pinned ffmpeg the first time it needs to encode.
...
| The Chromium download host | Set `PLAYWRIGHT_DOWNLOAD_HOST` to your mirror, as Playwright documents. DeckTalk fetches Chromium through Playwright wherever it fetches it. |
...
| The ffmpeg download | Put an approved `ffmpeg` and `ffprobe` on the machine and name them in `DECKTALK_FFMPEG` and `DECKTALK_FFPROBE`. Those two override every other ffmpeg source, so nothing downloads a build. |
