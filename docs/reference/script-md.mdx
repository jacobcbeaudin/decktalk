...
`cues.json` match spoken words. The scaffold writes "decktalk dot app" in the script and
shows decktalk.app on the slide. A cue on "decktalk dot app" is reliable, and a cue on
"decktalk.app" matches nothing, because matching drops the dot and leaves one token.
