...
{/* sample: decktalk verify, build without voice of a fresh starter, three rows of the cue table */}
```text
...
{/* sample: decktalk verify, build without voice of a fresh starter, recordings table */}
```text
sec  recording
 01  ok
 02  ok
...
{/* sample: decktalk verify, build without voice of a fresh starter, start table */}
```text
...
{/* sample: decktalk verify, build without voice of a fresh starter, cut table */}
```text
sec   cut at  before cut  result
 01    14.29   -120.0 dB  quiet
 02    30.17   -120.0 dB  quiet
 03    43.36   -120.0 dB  quiet
```

`cut at` is where the section's narration ends in the final mp4. The check measures the last samples of the
section's span in the narration track, which are exactly the samples the viewer hears before `cut at`.

```text
cut at         = end of the section's span on the narration clock + its run's offset
...
A clip between page sections, or a page section's `hold_seconds`, pauses the narration. The page sections
between two pauses make one run, and the run plays from where its first section starts in the final mp4. A
run's offset is that start minus the run's start on the narration clock, and the mix places the narration
with the same offsets. Each section's picture is cut on a whole frame, so the picture cuts within half a frame
of `cut at`, or after the section's hold.
...
- A page section's `hold_seconds` comes after its `cut at`. In the sample, the starter's section `03`
  sets `hold_seconds = 1.5`, so it holds its last slide from 43.36 s to the end of the video.
...
{/* sample: decktalk verify, build without voice of a fresh starter with seamless = true added to sections 2 and 3, continuity table only */}
```text
...
{/* sample: decktalk verify 2:2.1code, build without voice of a fresh starter, cue table only */}
```text
check                 cue       at   chg %   ctl %   offset     a/v  result
2:2.1code            9.43    23.71   10.79    0.00    +10ms    -1ms  changed
```
...
{/* sample: decktalk verify 2:2.1formula, build without voice of a fresh starter with offset 0.14 added to that cue, one row of the cue table */}
```text
check                 cue       at   chg %   ctl %   offset     a/v  result
...
{/* sample: decktalk verify 2:2.1formula --json, build without voice of a fresh starter with offset 0.14 added to that cue, the cues array only */}
```json
    "cues": [
      {
        "section": 2,
