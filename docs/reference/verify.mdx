...
- The last page section's `hold_seconds` comes after its `cut at`. In the sample, section `09` holds its last
...
<Tabs>
  <Tab title="Silent build">
    {/* sample: decktalk verify 8:7.1checked, silent build of the scaffold, cue table only */}
    ```text
    check                 cue       at   chg %   ctl %   offset     a/v  result
    8:7.1checked        10.03   238.31    6.56    0.00    +10ms    +3ms  changed
    ```
...
Example: the `chg %` of 6.56 for `8:7.1checked` is about 8,500 of 129,600 pixels.
...
Example: `8:7.1checked` has a changed share of 6.56 and a control share of 0.00. Its margin is 6.56, so it
passes.
Its 0.7 s probe has a margin of 0.83, and its 1.5 s probe has a margin of 6.56. `verify` reports the 1.5 s probe.
...
The figure's shares use `onset_diff_level` (12), so the onset frame reads 1.24%. The `chg %` column reads 6.56%,
...
{/* sample: decktalk verify 8:7.1checked --json, silent build of the scaffold, the cues array only */}
```json
    "cues": [
      {
        "section": 8,
