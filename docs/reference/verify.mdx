...
`decktalk verify` is the one read-only checker after a build. It repeats what each recording log judged,
and then checks the final mp4 in three ways: section starts, cuts, and cues. A section that sets
...
The verify stage inside `decktalk build` checks the recordings, section starts, cuts, and cut continuity,
and no cue. `decktalk verify` checks all five.
...
finding long after the build. The row is `PAGE ERROR`, `STALLED`, `TRUNCATED`, `NO COVER`, `BLACK?` or
`KATEX ERROR` or `KATEX NOT LOADED`, exactly as [`decktalk record`](/reference/cli#decktalk-record)
defines them, or `ok`. A
section with no recording log gets no row.
...
`OFF CUE`, `NO CHANGE`, `UNRESOLVED`, `UNCUED ELEMENT`, `BLACK`, `SPEECH AT CUT`, and `POP AT CUT` are certain findings, so `decktalk verify`
...
| `verify.recordings[]` | array | One object per recorded page section, with `key`, `where` (its recording log), `t0_method`, `verdicts` (verdict codes) and `page_errors`. |
