...
A named cue has the form `SECTION:CUE`, such as `4:4.1words`. `SECTION` is the section number, not the scene
...
{/* sample: decktalk verify, silent build of the scaffold, start table */}
```text
...
- A clip section has a row too. [Add a clip section](/guides/clip-section) shows one.

## Cuts

The cut check confirms that the narration is quiet just before each cut.

{/* sample: decktalk verify, silent build of the scaffold, cut table */}
```text
...
- The last page section's `hold_seconds` comes after its `cut at`. In the sample, section `05` holds its last
  slide from 202.30 s to the end of the video.
...
<Tabs>
  <Tab title="Silent build">
    {/* sample: decktalk verify 4:4.1words, silent build of the scaffold, cue table only */}
    ```text
    check                 cue       at   chg %   ctl %   offset     a/v  result
    4:4.1words          14.31   172.99    1.47    0.00    +10ms   -11ms  changed
    ```
...
Example: section 4 starts at 158.68 s, and `4:4.1words` has a cue time of 14.31 s. Its `at` is 172.99 s.
...
Example: the `chg %` of 1.47 for `4:4.1words` is about 1,900 of 129,600 pixels.
...
Example: for `4:4.1words`, the reference time is 172.99 - 0.14 = 172.85 s. The reference frame is the next frame on
the 40 ms grid, at 172.88 s. That frame is 110 ms before the cue, as the onset figure shows.
...
<Frame caption="Probes run forward from the reference, and two back-to-back control spans run back from it.">
  <img className="block dark:hidden" src="/images/verify-probes-light.svg" alt="A time axis around cue 4:4.1words. A dashed line marks the reference time, 0.14 s before the cue. Bars run from it to probes 0.7 s and 1.5 s after the cue, and each changed 1.47%. The 1.5 s probe is marked reported. Its two back-to-back control spans of 1.64 s run back from the reference, and each changed 0.00%." />
  <img className="hidden dark:block" src="/images/verify-probes-dark.svg" alt="A time axis around cue 4:4.1words. A dashed line marks the reference time, 0.14 s before the cue. Bars run from it to probes 0.7 s and 1.5 s after the cue, and each changed 1.47%. The 1.5 s probe is marked reported. Its two back-to-back control spans of 1.64 s run back from the reference, and each changed 0.00%." />
</Frame>
...
Example: `4:4.1words` has a changed share of 1.47 and a control share of 0.00. Its margin is 1.47, so it
passes.
Both of its margins round to 1.47. The margin at 1.5 s is larger by 0.0008, so `verify` reports the probe at 1.5 s.
...
<Frame caption="The outlined frame is the onset, 10 ms after the cue and inside the 80 ms offset limit.">
  <img className="block dark:hidden" src="/images/verify-onset-light.svg" alt="Seven frames 40 ms apart around cue 4:4.1words. The first is the reference frame, 110 ms before the cue. The frames at 70 and 30 ms before the cue changed 0.00%. The frame 10 ms after the cue changed 2.33% and is outlined as the onset. A shaded band spans 80 ms on each side of the cue." />
  <img className="hidden dark:block" src="/images/verify-onset-dark.svg" alt="Seven frames 40 ms apart around cue 4:4.1words. The first is the reference frame, 110 ms before the cue. The frames at 70 and 30 ms before the cue changed 0.00%. The frame 10 ms after the cue changed 2.33% and is outlined as the onset. A shaded band spans 80 ms on each side of the cue." />
</Frame>

The figure's shares use `onset_diff_level` (12), so the onset frame reads 2.33%. The `chg %` column reads 1.47%,
because the change test uses `diff_level` (40), at the probe 1.5 s after the cue.
...
Example: `4:4.1words` has an offset column of +10 ms, so its onset frame is at 173.00 s.
...
Example: `4:4.1words` has an offset column of +10 ms and an `a/v` of -11 ms, so its click distance is +21 ms.
...
| Reason | Cause | Fix |
|---|---|---|
| `OPTED_OUT` | `cues.json` marks the cue `"verify": false`, and the run named no cue. | Name the cue, such as `decktalk verify 4:4.1words`, to measure it anyway. |
...
{/* sample: decktalk verify 4:4.1words --json, silent build of the scaffold, the cues array only */}
```json
    "cues": [
      {
        "section": 4,
        "cue": "4.1words",
        "cue_seconds": 14.31,
        "final_seconds": 172.99,
        "changed_percent": 1.47,
        "control_percent": 0.0,
        "offset_ms": 10,
        "av_ms": -11,
        "verdict": "changed",
