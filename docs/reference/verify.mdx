...
- The last page section's `hold_seconds` comes after its `cut at`. In the sample, section `05` holds its last
  slide from 214.22 s to the end of the video.
...
Example: section 4 starts at 170.60 s, and `4:4.1words` has a cue time of 14.31 s. Its `at` is 184.91 s.
...
Example: for `4:4.1words`, the reference time is 184.91 - 0.14 = 184.77 s. The reference frame is the next frame on
the 40 ms grid, at 184.80 s. That frame is 110 ms before the cue, as the onset figure shows.
...
passes.
Both of its margins are 1.4668. On a tie, `verify` reports the earlier probe, at 0.7 s.
...
Example: `4:4.1words` has an offset column of +10 ms, so its onset frame is at 184.92 s.
