...
`record`, `measure` and `check` each return one row per section rather than one result.
...
| `preflight(project, *, only=None, frames=True, model=None, allow_unknown_cues=False)` | `PreflightResult` with `takes`, `note`, `align`, `estimated`, `cues`, `seams`, `frames`, `placeholders`, and `short` | `ConfigError`, `MissingInputError`, `ToolError` |
...
| `check(project, only=None)` | A list of `RecordingCheck`, each with `key`, `duration`, `wanted`, `y10`, `y50`, `y90`, `max50`, `verdicts`, `stall_ms`, `page_errors`, `file`, `ok`, `label`, and `to_dict(root)` | `MissingInputError` |
| `assemble(project, *, soundscape=True, loudness=True, strict=False)` | `AssembleResult` with `final`, `stamped` (`None` unless `[output] timestamped_copy` is on), `duration`, `sections`, `warnings`, `loudness`, `loudness_problems`, `captions_srt`, `captions_vtt`, and `chapters` | `ConfigError`, `MissingInputError`, `ToolError` |
| `screenshots(project, *, pages=None, slides=None, section=None, at=None, cues=None)` | `ScreenshotsResult` with `files` | `ConfigError` |
