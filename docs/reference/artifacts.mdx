...
```json
{
  ...
  "page_errors": [],
  "frame_gaps": [],
  "sync_log": [{ "text": "The curve rises, then th", "cueAt": 0.0, "runAt": 0.0, "n": 7, "firstOn": 0.0 }]
}
```

| Field | Meaning |
|---|---|
| `url` | The page URL the recorder opened, including `scene`, `beats`, `words`, and `t0=signal`. |
...
| `load_seconds`, `lead_seconds` | The recorder's wall-clock estimates of how long the page took to load and how long it waited before the clock started. |
...
| `sync_log` | One `{ text, cueAt, runAt, n, firstOn }` object per `data-sync` element that matched, copied from `window.__decktalk.syncLog`. |
