...
The index is also the narration clock. A section runs for its take plus the silence joined either side of
it, which is `duration_seconds` plus `lead_seconds` plus `tail_joined_seconds`, and the sections are joined
in key order. Where each one sits in `narration.mp3` is therefore arithmetic over the rows: its start is the
...
`duration_seconds + lead_seconds + tail_joined_seconds`. The spoken span runs from that start to the end of
the last word, which is the start plus `lead_seconds` plus `speech_end_seconds`.
