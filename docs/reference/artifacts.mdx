...
The out directory holds each cut section and everything a viewer receives: the final video, its
captions, its chapters, its cut list, its transcript page and its poster.
...
| `<name>.mp4` | The final video, with one chapter per section, and both streams tagged with `[project] language`. Consecutive sections with the same title share a chapter. |
...
```json
{"ts": "2026-09-18T11:04:02.311Z", "pid": 4821, "stage": "record", "stage_index": 3, "stage_count": 5, "section": 4, "event": "done", "detail": "Recorded the section: ok."}
```
