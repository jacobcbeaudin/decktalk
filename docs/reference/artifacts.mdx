...
your own tools may read any of them. Every JSON file is written atomically, so a reader
...
| `frame_gaps` | `[seconds, ms]` pairs, one for every gap between two page frames longer than 100 ms, copied from `window.__decktalk.frameGaps`. `check` reports `STALLED` past `stall_ms`. |
