...
| `sections.NN.tail_seconds` | number | Silence placed after `sound_end_seconds`: the section's `tail_seconds`, else `[narration] min_tail_seconds`. The join cuts the take at `sound_end_seconds` and places this silence after it, and the mp3 is not changed. |
