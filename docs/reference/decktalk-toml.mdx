...
| `title` | string | empty | Chapter title and slate title. An empty title becomes `Section N`. Consecutive sections with the same title share one chapter. |
| `carries_previous` | boolean | false | The section opens on the previous section's last picture. `verify` flags a visible jump at the cut as `POP AT CUT`. Not allowed on the first section. |
...
| `lead_seconds` | number | 0 | Seconds of silence before the section's first word. |
| `tail_seconds` | number | `[narration] min_tail_seconds` | Shortest silence after the section's last word. |
...
Three keys add silence to a page section. None of them is sent to the voice or changes the narration cache,
so a cached take stays cached.

| Key | Where the silence goes | How DeckTalk adds it |
|---|---|---|
| `lead_seconds` | Before the first word | `narrate` joins it into `narration.mp3` before the take. The take is not changed. |
| `tail_seconds` | After the last word | `narrate` pads the take, as it does for `min_tail_seconds`. |
...
Set `carries_previous = true` when a section continues the previous section's picture, such as a page that
