...
| `tail_seconds` | After the last word | `narrate` measures where the take's sound ends, cuts the take there in `narration.mp3`, and places exactly this much silence after it. The take is not changed. |
...
- `tail_seconds` is exact, like `min_tail_seconds`. Whatever the voice left after its last sound, a pause or a
  breath, is cut, so every take of the section ends on the same silence, voiced in this run or reused.
