...
`decktalk init` writes a six-section file with comments. This fuller example adds a clip section, a
...
`scene`, and a `words` key replaces the spoken words. `t0` is always `signal`, whatever `params` says.
...
The scaffold's section 4 sets `optional = true`. Delete that line after you add the clip.
