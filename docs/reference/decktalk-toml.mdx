...
`decktalk init` writes a three-section file with comments, and every section of it is a page section. This fuller
example adds a clip section, a dip to black, and music.
...
| `tail_seconds` | After the last word | `narrate` pads the take, as it does for `min_tail_seconds`. A take in a shared [`cache_dir`](/reference/configuration#narration) is never rewritten, so its tail is joined into `narration.mp3` after it instead. |
...
Note: the keys work in upper or lower case. A build without voice skips the loudness pass.

### `markers.json`

A markers file swells or mutes the music at spoken phrases. `[mix] music_markers` names it, and the
