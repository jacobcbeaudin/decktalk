...
| install | `decktalk install`. Fetches headless Chromium and ffmpeg up front, which a command that needs one of them would otherwise fetch itself, and on Linux installs Chromium's system libraries. | [CLI](/reference/cli#decktalk-install) |
