...
and exits 1 when a tool is missing. The config row is `ok` whether or not the file
...
| `STALLED` | The page went longer than `stall_ms` between two frames while recording, so a reveal may have been captured late. The recorder already retried up to `retries` times. |
| `PAGE ERROR` | The page threw a script error or exposed no `window.__decktalk.catalog`, so the section recorded a blank stage. `check` fails on it, and so does `build`. |
