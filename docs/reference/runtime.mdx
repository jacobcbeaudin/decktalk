...
section before, and leaves it out for the first section or when the section before has no words.
...
`render` function that throws is an uncaught page error. The recorder logs it, and `record` reports
`PAGE ERROR`.
