...
| Freeze mode | `frozen` | `?step=ID`, optionally with `&cue=CUE` or `&before=CUE` | at `load`, after the step has mounted | one step, with its reveals in their end state | `decktalk shots`, `decktalk preflight` |
...
| `before` | string | none | With `step` and no `cue`, fires the step's cues before this one. Elements of this cue and later cues stay hidden. |
...
| `catalog` | array | One `{ scene, name, steps, cues }` object per scene. `steps` lists the step ids. `cues` maps each step id to its cue ids in autoplay order. |
