...
{/* copied from template/deck/index.html, scene 4, with most of the markup and the handlers cut */}
...
DeckTalk.scene(4, { name: "The edit", steps: [
  { id: "4.1", hold: 25,
    ...
    render: () => `
    <div class="slide s4">
      <p class="s4-cap" data-cue="4.1lower" data-sync>The height is the error, so lower is better.</p>
      <div class="s4-verify" data-cue="4.1moved" data-fx="none">…</div>
    </div>` },
]});
</script>
```
...
`?step=4.1&cue=4.1lower` shows the script diff and the rebuild strip. It hides the verify rows,
which wait for `4.1moved`. `decktalk shots --step 4.1 --cue 4.1lower` opens that URL.
