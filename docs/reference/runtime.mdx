...
<ResponseField name="frameGaps" type="array">
  One `{ at, ms }` object for every gap over 100 ms between two animation frames, where `at` is seconds since narration t=0. The recorder copies it to the sidecar as `frame_gaps`, and `check` reports `STALLED` past `stall_ms`.
</ResponseField>
