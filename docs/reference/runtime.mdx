...
<ResponseField name="step" type="string">
  Freeze mode. Mount this step with every reveal in its end state and its handler cues fired. Animations run with zero duration, count-ups show their final value, and typewriters show their full text.
</ResponseField>
...
<ResponseField name="reveal(element)" type="function">
  Reveals one element now, with its count-up or typewriter, as a cue would. An element already revealed is left alone.
</ResponseField>
...
<ResponseField name="data-fx" type="rise | fade | draw | drop | pop | dim | none" default="rise">
  The reveal animation. [The page contract](/concepts/page-contract#reveal-effects) says what each one is for. `draw` needs an SVG path with `pathLength="1"`.
</ResponseField>

<ResponseField name="data-dur" type="number">
  The animation length in seconds. It also sets the count-up length, which otherwise takes 0.9 seconds.
</ResponseField>
...
<ResponseField name="data-type" type="number" default="40">
  Type the text at this many milliseconds per character on reveal. The element is emptied first, so give its container a fixed size or a background if the layout must not jump.
</ResponseField>
