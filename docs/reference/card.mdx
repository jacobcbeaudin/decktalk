...
| `deck/index.html` + `deck/decktalk-runtime.js` | `DeckTalk.scene(N, {name, camera?, steps: [{id, hold?, cues?, render, enter?, on?}]})`. Reveals use `data-cue="id"`, `data-at="s"`, `data-fx`, `data-dur`, `data-count`, `data-type`, `data-sync` (one word at a time as the voice says it, text word for word as spoken, with `data-fx="none"`), `data-tex` with `data-display`. Inside the render backticks every backslash is doubled. The stage is 1920 by 1080. [Runtime](/reference/runtime), [Page contract](/concepts/page-contract). |
...
decktalk verify 3:3.1eq 5:5.1a              # measure each cue's offset in the final mp4
...
carries a click at every word start, and the cue table gains an `a/v` column, the
picture's first change minus the click nearest the cue, which fails past
`max_offset_frames`, and a silent build's `a/v` column within `max_av_frames`. [CLI](/reference/cli) lists every flag.
