...
Every key but `cue` and `on` is optional. This sample carries all of them.

```json cues.json
{ "sections": { "2": { "min_seconds": 12, "cues": [
  { "cue": "2.1average", "on": "the average" },
  { "cue": "2.1formula", "on": "the formula", "occurrence": 2, "offset": 0.2 },
  { "cue": "2.1code", "on": "in code", "case_sensitive": true },
  { "cue": "2.1waited", "on": "$end", "verify": false }
] } } }
```
...
decktalk install                            # optional: fetch Chromium and ffmpeg up front, and Chromium's Linux libraries
