...
- The default `min_tail_seconds` is 0.7. The scaffold sets 0.45.
...
- A clip section adds `clip`, and the optional `slate_seconds` and `optional`.
...
- Only the last page section can set `hold_seconds`.
...
- A section can play a scene with another number. Scaffold section 5 plays scene 4.
...
decktalk verify 1:1.1bowl --cue 6:5.1cap    # only these two cues
decktalk verify --json                      # the same result as one JSON object on stdout
decktalk shots                              # one PNG per step
decktalk shots --step 4.1 --cue 4.1lower    # step 4.1 frozen at cue 4.1lower
decktalk shots --section 5 --at 12.5        # a frame 12.5 s after narration t=0 in section 5
decktalk status [--json]                    # what exists and what is built
decktalk narrate --dry-run                  # the text each section would send. Sends nothing.
...
- A repeatable option takes one value each time. Write `--only 3 --only 5`.
- `--strict` on `assemble` and `build` fails on a missing recording or a missing clip.
- A clip section with `optional = true` still plays its slate under `--strict`. The scaffold's section 4 sets it.
...
- `build` checks section starts and cuts. It measures no cue.
...
| `build/out/<name>.chapters.txt` | `assemble` | One chapter per section. |
