...
- To gate on the whole video, run `decktalk build --no-voice && decktalk check && decktalk verify`.
...
|---|---|---|
| `build/narration/NN-slug.mp3`, `NN-slug.words.json` | `narrate` | One section's audio and its `[{word, start, end}]` list. |
...
| `build/cue-times.json` | `align` | `{"NN": "cue@seconds,…"}`, in seconds after the section start. |
| `build/cue-times.anchors.json` | `align` | Each cue's word start without its offset key. `verify` uses it to find the click. |
