...
decktalk status [--json]                    # what the input files say and what is built
...
| `--strict` | `align`, `preflight`, `check`, `assemble`, `verify`, `clip`, `build` | Also exits 1 on an uncertain finding. |
| `--exit-zero` | the same commands and `doctor` | Exits 0 on findings. An error still exits 3. |
