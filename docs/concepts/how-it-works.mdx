...
| `result` | `changed` when the best probe changed at least `min_changed_percent` of the pixels and beat its control by `min_margin_percent`, and the offset, and the `a/v` value when there is one, are within `max_offset_frames` of the cue. `OFF CUE` when the picture changed but the onset sits further from the cue than that. `NO CHANGE` when the picture did not change enough. |
