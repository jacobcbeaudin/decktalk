...
<Frame caption="Each panel names the pipeline stages it runs, in order.">
  <picture className="block dark:hidden">
    ...
    <img src="/images/how-it-works-light.svg" alt="Four panels. Write shows a markdown script. Narrate runs narrate and beats, and shows a tick for every word with 1.25 over the first cue word. Record runs record, measure, and check, and shows a slide. Assemble runs assemble and verify, and shows one mp4." />
  </picture>
  <picture className="hidden dark:block">
    <source media="(max-width: 640px)" srcSet="/images/how-it-works-dark-stacked.svg" />
    <img src="/images/how-it-works-dark.svg" alt="Four panels. Write shows a markdown script. Narrate runs narrate and beats, and shows a tick for every word with 1.25 over the first cue word. Record runs record, measure, and check, and shows a slide. Assemble runs assemble and verify, and shows one mp4." />
  </picture>
</Frame>
...
flowchart TB
  S["script.md"] --> narrate
  narrate -- "words files, timeline.json" --> beats
  C["cues.json"] --> beats
  beats -- "beats.json" --> record
  P["deck pages"] --> record
  record -- "webm and sidecar" --> measure
  measure -- "lead_in_seconds" --> check
  check --> assemble
...
|---|---|---|
| `narrate` | `narrate` voices each section and gets a time for every word. | `build/audio/NN-slug.mp3`, `NN-slug.words.json`, `manifest.json`, `narration.mp3`, `timeline.json` |
| `beats` | `beats` turns each cue phrase into a cue time. | `build/audio/beats.json`, `beats.anchors.json` |
| `record` | `record` records each page section in headless Chromium. | `build/rec/NN-scene.webm`, `NN-scene.json` |
| `measure` | `measure` finds narration t=0 in each recording. | `lead_in_seconds` in each sidecar |
| `check` | `check` flags a recording that is black, short, stalled, or uncovered. | nothing |
| `assemble` | `assemble` cuts, joins, and mixes the sections into one mp4. | `build/out/NN-section.mp4`, `<name>.mp4`, captions, chapters |
| `verify` | `verify` checks section starts and cuts in the final mp4. | nothing |
...
| Joined audio adds a short delay at each join. | Each `NN-section.mp4` has no audio. `assemble` mixes the sound once, from zero. |

The recorder removes the cover only when the page is ready. [The handshake](/concepts/page-contract#the-handshake)
shows each wait. `measure` looks for the cover only in the first `[align] scan_seconds` of each recording, 4 s by
...
{/* sample: decktalk build --silent, silent build of the scaffold (R2), first narrate lines */}
```text
[sil ] 01-open.mp3  29 words -> 16.99s (estimated words)
[sil ] 02-how-it-works.mp3  39 words -> 19.58s (estimated words)
[sil ] 03-how-ai-learns.mp3  228 words -> 115.39s (estimated words)
```
...
- Every other stage runs as usual. A silent build therefore tests the cues, the pages, the recording, the cut, and
  the checks.
...
A silent build after a voiced build empties the narration cache.
[A silent build and a voiced build](/guides/rebuild-one-section#a-silent-build-and-a-voiced-build) explains the cost.
...
- **Cues:** the picture changes within 80 ms of each cue time. After a silent build, it also changes within 120 ms
  of the click plus the offset key.

The verify stage inside `decktalk build` checks section starts and cuts. `decktalk verify` also checks the cues.
...
| Term | Meaning |
|---|---|
| [Stage](/reference/glossary) | One of `narrate`, `beats`, `record`, `measure`, `check`, `assemble`, and `verify`, in that order. |
| [Silent build](/reference/glossary) | A build with `--silent`. It uses estimated word times and writes a click track. |
| [Voiced build](/reference/glossary) | A build without `--silent`. The speech provider voices the script. |
| [Click track](/reference/glossary) | The audio of a silent build, with a soft click at every estimated word start. |
