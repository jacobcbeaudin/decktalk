...
| `verify` | Confirms that every section opens on a real frame, that the narration is quiet in the last 150 ms before every cut, and that each cue in `beats.json` changed the picture on time. After a silent build it also measures the picture against the placeholder click at the cued word. Inside `build` it checks starts and cuts only. | nothing |
...
`"verify": false`. Naming cues as `SECTION:CUE` measures only those, even a marked one.
