...
for a project that has a music file. DeckTalk computes the level from the scaffold's `[mix]` and `[audio]` settings.

<Frame caption="The music rises as the speech of section 4 ends, then stays down under the clip.">
  <img className="block dark:hidden" src="/images/duck-lane-light.svg" alt="The music level around clip section 5, from 3:14 to 3:25 of the video. It sits at −30 dB under each spoken span and under the whole clip. It ramps up to −24 dB over the last 0.5 s of speech in section 4, and briefly at the cut into section 6." />
  <img className="hidden dark:block" src="/images/duck-lane-dark.svg" alt="The music level around clip section 5, from 3:14 to 3:25 of the video. It sits at −30 dB under each spoken span and under the whole clip. It ramps up to −24 dB over the last 0.5 s of speech in section 4, and briefly at the cut into section 6." />
</Frame>

## Markers

A markers file swells or mutes the music at spoken phrases. It gives the music some structure without an
edit to its file. The scaffold's `media/markers.json` swells the music at the start of section 1.
...
2. It applies the gain that reaches the integrated target, `[mix.loudness] I`.
...
{/* sample: decktalk build --no-voice, build without voice of the scaffold (R2), assemble log line */}
```text
...
{/* sample: decktalk soundscape --dry-run music, scaffold with [soundscape.music] uncommented */}
```text
...
and it writes to the `[mix] music` path. Leave that table in comments, or point `[mix] music` at your own
file.
