...
The next figure shows the underscore level around the clip of the scaffold, for a project that has an underscore
file. DeckTalk computes the level from the scaffold's `[mix]` and `[audio]` settings.

<Frame caption="The underscore rises as the speech of section 3 ends, then stays down under the clip.">
  <img className="block dark:hidden" src="/images/duck-lane-light.svg" alt="The underscore level around clip section 4 of the scaffold. It sits at −30 dB under each spoken span and under the whole clip. It ramps up to −24 dB over the last 0.5 s of speech in section 3, and briefly at the cut into section 5." />
  <img className="hidden dark:block" src="/images/duck-lane-dark.svg" alt="The underscore level around clip section 4 of the scaffold. It sits at −30 dB under each spoken span and under the whole clip. It ramps up to −24 dB over the last 0.5 s of speech in section 3, and briefly at the cut into section 5." />
</Frame>
...
{/* STALE SAMPLE: regenerate from a v11 build (decktalk soundscape --dry-run on the scaffold). */}
```text
...
that has no manifest file next to it, and it overwrites your file. The scaffold's `[soundscape.music]` writes to
the `[mix] underscore` path. Delete that table, or point `[mix] underscore` at your own file.
