...
description: Every key in decktalk.toml, and cues.json.
---

One file per project. Paths are relative to the project directory. An unknown top-level
table fails the load, so a typo cannot silently do nothing.

Five words the docs use:

- **section**: one `## N.` block of `script.md` and its `[[section]]` table; the unit of narration, recording and cutting.
- **scene**: what a page plays for one section: `DeckTalk.scene(id, …)`, selected with `?scene=`.
- **step**: one slide state inside a scene; `?step=ID` freezes it.
- **cue**: a named moment. In `cues.json` it is a spoken phrase to find; in the page it is an id that reveals elements or runs handlers.
- **beat**: a cue resolved to seconds after the section's narration starts (`id@s`), stored in `beats.json` and passed to the page as `?beats=`.
...
| Key | Default |
|---|---|
| `provider` | `elevenlabs` (the only one today; see the Python API for adding one) |
| `model` | tool default |
| `stability` | 0.55 |
...
Prompts for `decktalk soundscape`. Ambience and music write to the `[mix] ambience` and `underscore` paths (or `build/sfx/ambience.mp3` and `build/music/underscore.mp3` when `[mix]` names none); each sfx writes to `build/sfx/<name>.mp3` unless `out` is set.
...
`step` (or its synonym `cue`) is a cue id the page understands; `on` is a spoken phrase from that section
