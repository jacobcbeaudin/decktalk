...
| `beats=id@s,id@s,…` | Beat mode: cues resolved to times. Fire cue `id` at `s` seconds after narration t=0. Times come from `build/audio/beats.json`, relative to the section's start in the continuous narration. |
...
Markup attributes inside a step: `data-cue`, `data-at`, `data-fx` (`rise` by default, `fade`, `draw` for SVG paths with `pathLength="1"`, `drop`, `pop`, `dim`, `none`), `data-dur` (seconds; a count-up defaults to 0.9),
`data-count` (`first` to count the first number instead of the last), `data-type`
(ms per character, default 40), `data-tex` (+ `data-display` for display math).
...
last magenta frame in the webm and writes `lead_in_seconds` to the sidecar. The assembler
trims that lead-in, so video t=0 equals narration t=0 to within a frame.
