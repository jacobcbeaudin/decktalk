...
well. The scaffold names every cue in its step's `cues` object. In autoplay that object
...
| Effect | For | Against |
|---|---|---|
| `rise` (default) | Lines of text, labels, and cards. It moves up 10 px as it fades in over 0.3 s, which reads as a line arriving. | Code, tables, and anything aligned to a grid, where the motion looks like a layout shift. |
