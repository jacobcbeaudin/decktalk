...
attribute with its default. This guide explains how the pieces fit.
...
never loaded or that a `data-tex` value could not be parsed. [How it works](/concepts/how-it-works#why-the-cuts-are-exact) explains the
...
well. The scaffold names every cue in its step's `cues` object, which also gives the
browser preview its autoplay timing.
...
| Element | Size |
|---|---|
| Safe margin | 96 px from every edge, and 160 px for titles. The scaffold sets 120 px side margins. |
| Body text | 36 px or larger. |
| Captions | 30 px, in the muted color. |
| Headlines | 64 px to 120 px. The scaffold's hero is 120 px and its titles 64 px. |
| Eyebrows and labels | 22 px, in the monospace face and the muted color. |
