...
The recorder covers the page in magenta from its first paint. It opens the page with
`t0=signal`, waits for `load`, awaits `__sceneReady`, waits the settle time and at least
...
on every platform. [How it works](/concepts/how-it-works#why-the-cuts-are-exact) has the
picture.
