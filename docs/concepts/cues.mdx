...
<path class="curve" data-cue="1.1bowl" data-reveal="instant" d="${s1curve()}"/>
```
</CodeGroup>

1. You write the cue phrase in `cues.json`, and the cue id in the page.
2. The `align` stage resolves each cue phrase to a cue time, in seconds after the section start. The cue time of `1.1bowl` is the start of the word "bowl".
...
`decktalk align` prints the cue times of every section. These lines come from a build without voice, so
the times are estimates.

{/* sample: decktalk align, build without voice of the scaffold, trimmed to section 1 */}
```text
...
removed from `deck/index.html`. `align` exits 1.

{/* sample: decktalk align, build without voice of the scaffold with cue id 5.1cap removed from the page, trimmed to section 9 */}
```text
sec  speech   need  cues
...
run time, pass `--allow-unknown-cues`. The page warnings are then the only check.
...
wrote, so the reveal never fires.

```text
sec  speech   need  cues
 09    11.4      -  5.1clear@0.91,5.1made@4.1,5.1url@10.03
                    ! 5.1cap: data-cue="5.1cap" in deck/index.html has no entry in cues.json
```
...
{/* sample: decktalk align, build without voice of a fresh scaffold with a typo in the 3.4name phrase, trimmed to section 3 */}
```text
sec  speech   need  cues
...
mounts. Fix the phrase, or pass `--allow-unresolved-cues` to build without the cue. The page then warns
that the cue is not in `?cues=`, and its element appears at its `data-delay` time.
