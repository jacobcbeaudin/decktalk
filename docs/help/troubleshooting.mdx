...
| `need build/sections/NN.mp4 files` | `verify` | [Entry](#need-build-sections-nn-mp4-files-in-verify) |
...
```text
decktalk.toml: [voice]: ignoring unknown key 'stabilty' (did you mean 'stability'?). docs/reference/renames.mdx lists every name DeckTalk renamed.
```
...
sec  speech   need  cues
 01    13.2      -  1.1title@0.7,1.1script@3.1,1.1deck@6.47,1.1word@11.76
 02    14.1      -  2.1average@0.94,2.1code@8.93,2.1waited@10.81
                    ! 2.1formula: phrase not found: 'the formular'
 03    11.4      -  3.1idea@0.91,3.1again@7.3,3.1make@8.21
...
```text
 02    14.1      -  2.1average@0.94,2.1formula@5.17,2.1code@8.93,2.1waited@10.81
...
error[UNKNOWN_CUE]: 1 cue id(s) in cues.json appear nowhere in the page that plays them, so the page would never reveal them. Add data-cue="2.1code" to the slide in deck/index.html, fix the id in cues.json, or pass --allow-unknown-cues:
...
```text
section 04: media/intro.mov missing; slate for 4s (drop your clip at that path; the section is optional, so --strict allows the slate)
```
...
3:3.1make            8.21    38.13   10.84    0.00    -90ms  -118ms  OFF CUE
```
...
```text
error[MISSING_INPUT]: need build/sections/NN.mp4 files and the final mp4; run `decktalk assemble` first
```
...
```text
error[CONFIG]: section 4 is a clip section; cut a clip from a page section
```
...
{/* sample: decktalk clip 1 --start 0.9 --end 2.0 --out media/cut.mp4 after decktalk build --no-voice on a fresh starter */}
...
seconds 1.12, words 1 | 0 certain, 2 uncertain finding(s)
```
...
A build without voice of the starter took 64 s for its 44.12 s video. The machine was a MacBook Pro
...
{/* sample: decktalk screenshots --section 1 --at 31 after decktalk build --no-voice on a fresh starter */}

```text
error[INTERNAL]: Page.wait_for_function: Timeout 30000ms exceeded.
```
