...
|---|---|---|
| `this is a musl system` | `install.sh` | [Entry](#this-is-a-musl-system-in-install-sh) |
| `playwright install failed` | `install`, and any command that fetches Chromium | [Entry](#playwright-install-failed-in-install) |
| `Chromium was fetched and still would not launch` | any command that opens a page | [Entry](#chromium-was-fetched-and-still-would-not-launch) |
...
### `this is a musl system` in install.sh

**Cause.** The machine uses musl rather than glibc, and Playwright publishes no musllinux wheels, so
DeckTalk cannot be installed on it at all. Alpine is the usual one.

**Fix.**
1. Install on a glibc distribution instead.
2. If this is a container image, build from `debian`, `ubuntu`, or `python:3.13-slim` rather than an
   Alpine image.

**More.** [glibc, not musl](/requirements#glibc-not-musl)
...
the error shows the reason. Any command that needs a browser fetches Chromium, so this can come from a
build on a machine where nothing was installed by hand, not only from `decktalk install`.
...
**More.** [What DeckTalk downloads](/requirements#what-decktalk-downloads)

### `Chromium was fetched and still would not launch`

**Cause.** The browser is on the machine and cannot start. On Linux that is almost always its system
libraries: a build fetches Chromium without them, because installing them goes through sudo and a build
that stops for a password is a build that hangs in CI. The message carries the line Chromium printed,
and a missing library names itself in it, such as `libnss3.so`.

**Fix.**
1. Run `decktalk install`, as a user who can use sudo. It is the one command that installs the libraries.
2. If the distribution has no apt-get, install Chromium's dependencies with its own package manager, or
   record with a browser the machine already has by naming it in `[record] browser_path`.
3. Run the build again.

**More.** [Linux system libraries](/requirements#linux-system-libraries)

### `MISSING` in doctor

**Cause.** A component is not installed. The detail says which one, and what would install it. `doctor`
downloads nothing itself, so a machine that has never built anything yet shows rows a build would fill in.
- If `chromium` shows `MISSING`, Chromium does not launch. The detail says whether it is simply not
  fetched yet, which the next build does, or on disk and refusing to start, which `decktalk install` fixes.
...
**Cause.** The narration in the last 0.15 s of a section's span is louder than -40 dBFS. The voice was still
speaking when the section's narration ended. The finding names the cut into the next section, its hold, or
the end of the film, and the time where the narration ends.
