...
| `hold_seconds is allowed only on the last page section` | every project command | [Entry](#hold_seconds-is-allowed-only-on-the-last-page-section-in-decktalk-toml) |
...
### `hold_seconds is allowed only on the last page section` in decktalk.toml

**Cause.** A page section other than the last page section sets `hold_seconds`. A hold there would move
every later picture off its words.

**Fix.**
1. Move `hold_seconds` to the last page section.
2. If you need a pause in the middle, add words or a `[pause N]` to the script instead.

**More.** [Page section keys](/reference/decktalk-toml#page-section-keys)
