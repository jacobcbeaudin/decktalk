...
{/* sample: decktalk narrate --silent --only 6 on a fresh scaffold */}

```text
error: no spoken sections match [6]; spoken sections are [1, 2, 3, 4, 5]
```
...
error: 1 cue id(s) in cues.json appear nowhere in the page that plays them, so the page would never reveal them. Add data-cue="5.1cap" to the step in deck/index.html, fix the id in cues.json, or pass --allow-unknown:
  section 05: 5.1cap: not in deck/index.html
```
...
{/* sample: decktalk assemble in clip-lesson from Add a clip section, with optional = true and no media/chips.mp4 */}
...
**Fix.**
1. Write each cue as `SECTION:CUE`, such as `4:4.1words`.
...
{/* sample: decktalk record --only 5 on a scaffold copy with data-cue="5.1cap" removed from deck/index.html */}

```text
[page] 05-scene  cue "5.1cap" matches no element, handler, or step
```
...
A silent build of the scaffold took 224.4 s for its 204.84 s video. The machine was a MacBook Pro
