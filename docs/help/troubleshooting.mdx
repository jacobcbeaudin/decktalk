...
{/* sample: decktalk narrate --silent --only 4 on a fresh scaffold */}

```text
error: no spoken sections match [4]; spoken sections are [1, 2, 3, 5, 6]
```
...
error: 1 cue id(s) in cues.json appear nowhere in the page that plays them, so the page would never reveal them. Add data-cue="5.1cap" to the step in deck/index.html, fix the id in cues.json, or pass --allow-unknown:
  section 06: 5.1cap: not in deck/index.html
```
...
**Fix.**
1. Open `build/rec/NN-scene.json` and read `lead_method`.
2. If it starts with `NO COVER`, follow [`NO COVER` in measure or check](#no-cover-in-measure-or-check).
...
{/* sample: decktalk assemble after a silent build of the scaffold, section 4 with optional = true and no clip file */}
...
**Cause.** The file that `clip` names does not exist, so a titled slate plays for `slate_seconds`. The
scaffold's section 4 does this until `media/broll-chips.mp4` exists, and that is expected.

The scaffold's section 4 sets `optional = true`, so its warning ends with
...
{/* sample: assemble stage of decktalk build --silent on a fresh scaffold (A-qs-02) */}
...
it. The scaffold names both files, and `decktalk soundscape` generates them.
...
**Fix.**
1. Write each cue as `SECTION:CUE`, such as `5:4.1words`.
...
{/* sample: record stage of decktalk build --silent on a scaffold copy with data-cue="5.1cap" removed from deck/index.html */}

```text
[page] 06-scene  cue "5.1cap" matches no element, handler, or step
```
...
A silent build of the scaffold took 216.6 s for its 197.96 s video. The machine was a MacBook Pro
