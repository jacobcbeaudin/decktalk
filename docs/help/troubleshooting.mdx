...
| `RUNTIME_OUTDATED` | `preflight` | [Entry](#runtime_outdated-in-preflight) |
...
**More.** [Runtime](/reference/runtime#url-parameters)
...
```text
error: ~/my-lesson/build/audio/timeline.json not found. Run `decktalk narrate` first.
```
...
{/* sample: decktalk clip 1 --from 2 --to 8 --out media/count.mp4 on a fresh scaffold with no build */}

```text
error: section 1 has no section video at ~/my-lesson/build/out/01-section.mp4. Run `decktalk assemble` first.
```
...
{/* sample: decktalk clip 5 --from 0 --to 1 --out media/x.mp4 on a fresh scaffold */}
...
{/* sample: decktalk clip 1 --from 2.5 --to 8.32 --out media/cut.mp4 after decktalk build --silent on a fresh scaffold */}

```text
section 01: the span cuts the word 'ball.' in two, so the words file leaves it out
```

**Cause.** `--from` or `--to` falls inside a spoken word. The clip keeps the part of the word inside the span, but the
