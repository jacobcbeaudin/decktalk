...
<AccordionGroup>
  ...
  <Accordion title="a/v offset: picture N ms from the click">
    ...
    The click and the cue come from the same word time, so a value beyond the a/v limit
    means the audio and the picture of the finished file drifted apart somewhere between
    the recording and the mix. Check that the section's sidecar reports a `cover` lead
  ...
  <Accordion title="Windows">
    Every push to `main` runs DeckTalk's unit tests and a full offline build on Linux. The
