...
- Chromium goes in `%LOCALAPPDATA%\ms-playwright`, fetched by the first command that needs it or by `decktalk install`. If a corporate proxy blocks the download, set `PLAYWRIGHT_DOWNLOAD_HOST` as Playwright documents.
