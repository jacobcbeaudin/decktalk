...
A cuecut page is an HTML file that `cuecut record` can drive from the narration. The
contract is a handful of URL parameters and one global. `cuecut-runtime.js` implements
...
| `step=ID` | Freeze mode. Mount step ID with every reveal in its end state and its handler cues fired. Used by `cuecut shots` and for review. |
...
- `window.__cuecut` — `{ mode, scene, step, cues, fired, catalog, now() }`. `catalog`
  lists every scene and its step ids (used by `cuecut shots`); `fired` is the list of cue
  ids fired so far (printed by `cuecut shots --section N --at S`).
...
4. When a cue fires: matching `data-cue` elements reveal, then the step's `on[id]`
   handler, then every `Cuecut.on(id, fn)` handler.
...
Cuecut.on("3.1draw", fn);               // global cue handler
Cuecut.start();                         // automatic on DOMContentLoaded when scenes exist
```
...
The runtime creates `#cc-stage` (1920×1080, scaled to fit the window) with `#cc-cam`
and `#cc-pan` inside; slides are `.cc-slide` children of the pan layer. Style the stage
and your slides however you like; the runtime's own CSS is prefixed `cc-`.

## Recording alignment

The recorder opens the page, waits for `load`, awaits `__sceneReady`, waits `t0`
seconds, then injects a full-frame magenta div for 120 ms. `cuecut measure` finds the
