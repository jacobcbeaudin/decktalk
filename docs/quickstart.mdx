  ...
  <Step title="Install DeckTalk" stepNumber={1}>
    ...
    <Tabs>
      <Tab title="One line">
        ...
        Pipe it into `DECKTALK_VERSION=0.4.1 sh` to install one version, or into `sh -s -- --dry-run` to print what would happen and change nothing. It runs on Linux and macOS. On Windows, use the uv tab. On Alpine and other musl systems it stops and says why: Playwright, which records the slides, publishes no musl wheels, so DeckTalk needs glibc.
      </Tab>
      <Tab title="uv">
        ```console
