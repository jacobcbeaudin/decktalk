...
<Steps>
  ...
  <Step title="Scaffold and build">
    ...
    script lights up word by word as the voice reads it, a scene that shows the four files
    that refer to each other by one number, a gradient descent lesson in which a three.js
    loss surface fades up and a ball steps down it while the update rule builds in KaTeX,
    a scene in which one sentence is retyped and the build table fills in, and an end
    card. All five are page sections, which DeckTalk records from
