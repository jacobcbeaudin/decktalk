...
- Step 2 is optional everywhere but Linux, where it installs Chromium's system libraries and needs sudo or root.
...
<Steps>
  <Step title="Install DeckTalk" stepNumber={1}>
    ...
    <Tabs>
      <Tab title="One line">
        ...
        It installs uv if you do not have it, then DeckTalk, and uv brings its own Python. It never runs sudo, and it never downloads Chromium or ffmpeg: step 2 does that, separately and visibly, and so does your first build. [Read the script](https://decktalk.ai/install.sh) before you run it. That is why it is served from a URL.
  ...
  <Step title="Download Chromium and ffmpeg now, or let the first build do it" stepNumber={2}>
    Nothing makes you run this. Every command that needs Chromium or ffmpeg downloads it the first time, and says so before the download starts. Run it here to get a few hundred megabytes out of the way while you read, to fill a Docker layer or a CI cache, or to set a machine up before it goes offline.

    On Linux, run it: it is also what installs Chromium's system libraries, through apt-get, and it is the one DeckTalk command that asks for a password. A build never does, because a build runs unattended.
    ...
    Check that the last line is `install complete`. On Linux, this step asks for sudo. [What DeckTalk downloads](/requirements#what-decktalk-downloads) says where each download goes.
  </Step>
  ...
  <Step title="Check the tools" stepNumber={3}>
    ...
    Check that the first line reads `missing 0` and that every row says `ok`. `doctor` reports and downloads nothing, so if you skipped step 2 the `chromium` and `ffmpeg` rows read `missing` until something fetches them, and each row says what would. The `config` row is the per-machine settings file, which is optional, so `none` there is a passing row.
  </Step>
</Steps>
...
<Steps>
  <Step title="Create the starter" stepNumber={4}>
    ...
    If you skipped step 2, the build in step 7 fetches Chromium and ffmpeg before it records, and prints a line for each download. `init` copies the packaged KaTeX into `deck/katex/` either way.
  ...
  <Step title="Build the video without an account" stepNumber={7}>
    A build without voice uses estimated word times and a click track, so it needs no key. Recording runs in real time. On a MacBook Pro with Apple M5 Pro and 64 GB memory, this build took about 64 seconds, on a machine that already had Chromium and ffmpeg. If you skipped step 2, the download comes first and the build names it as it starts. Read [What the starter shows](#what-the-starter-shows) while it runs.
