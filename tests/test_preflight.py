"""decktalk preflight's frozen frames, in a real headless Chromium with the ffmpeg that `decktalk setup` fetches.
...
def test_preflight_estimates_each_reveal_and_the_carried_cut_from_frozen_frames(tmp_path, monkeypatch):
    """Sections 1 and 2 of the scaffold, with section 2 set to carry the Open's last picture."""
    ...
    # The first cue of each section freezes its step just before the cue, and each later cue after the one before.
    assert rows["1:1.1bowl"].before.name == "step-1.1-before-1.1bowl.png"
    assert rows["1:1.1ball"].before == rows["1:1.1bowl"].after
    # Scene 2 opens on the Open's last picture, so the carried cut does not show.
    [carry] = result.carries
    assert (carry.key, carry.verdict) == ("02", "ok") and carry.changed_percent == 0.0
    assert carry.last == rows["1:1.1word"].after and carry.first == rows["2:2.1mark"].before


def test_preflight_only_checks_the_cut_into_a_carrying_section_it_names(tmp_path, monkeypatch):
    """`--only 2` on a section that carries section 1 still resolves section 1, but reports only section 2."""
    from decktalk.project import Project
    from decktalk.stages.narrate import narrate
    from decktalk.stages.preflight import preflight
    ...
    [carry] = result.carries
    assert (carry.key, carry.verdict) == ("02", "ok"), (carry.verdict, carry.reason, carry.detail)
    assert carry.changed_percent == 0.0 and carry.last is not None and carry.first is not None
    assert [t.segment.key for t in result.takes] == ["02"]
    assert {c.check.split(":")[0] for c in result.cues} == {"2"}
    assert set(result.beats.beats.sections) == {"02"}


SYNTH_PAGE = """
const box = (x, size, cue) => `<div ${cue ? `data-cue="${cue}" data-fx="none"` : ""}
  style="position:absolute;left:${x}px;top:200px;width:${size}px;height:${size}px;background:#000"></div>`;
DeckTalk.scene(1, { steps: [{ id: '1.1', cues: { '1.1in': 0, '1.1big': 1, '1.1mid': 2, '1.1dot': 3 },
  render: () => box(100, 400, '1.1in') + box(700, 400, '1.1big')
    + box(1300, 60, '1.1mid') + box(1500, 8, '1.1dot') }] });
DeckTalk.scene(2, { steps: [{ id: '2.1', cues: { '2.1go': 1 },
  render: () => box(1300, 400, '') + box(100, 60, '2.1go') }] });
DeckTalk.scene(3, { steps: [{ id: '3.1', cues: { '3.1go': 1 },
...
def test_preflight_reads_each_verdict_from_a_synthetic_page(tmp_path, monkeypatch):
    """A big reveal, a thin one, a dot, a cue at the start, a cut that pops, and a cut that carries its picture."""
    from decktalk.project import Project
    from decktalk.stages.preflight import preflight
    ...
    assert {k.key: k.verdict for k in result.carries} == {"02": "POP AT CUT", "03": "ok"}
    assert [k.changed_percent for k in result.carries][1] == 0.0
