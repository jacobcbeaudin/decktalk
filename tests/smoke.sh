...
uv run cuecut init "$T" --name smoke
# a synthetic "on camera" clip: 3 s, 1280x720 (exercises scale/pad), a 440 Hz tone as the voice
FF="$(uv run python -c 'from cuecut.tools import ffmpeg; print(ffmpeg())')"
...
uv run cuecut -p "$T" build --silent --preset veryfast
uv run cuecut -p "$T" shots
uv run cuecut -p "$T" shots --section 2 --at 1 --at 6
uv run cuecut -p "$T" verify 2:2.1c 3:3.1eq
uv run cuecut -p "$T" status
