...
class VerifyConfig:
    ...
    onset_percent: float = 0.01  # About a dozen changed pixels mark where a reveal begins, past motion noise.
    onset_diff_level: int = 12  # Luma steps for the onset scan only, so a fade or a low-contrast panel registers early.
    click_search_seconds: float = 0.25  # The click search covers this many seconds each side of the cued word's start.
