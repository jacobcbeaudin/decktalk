...
class Handler(http.server.SimpleHTTPRequestHandler):
    ...
    def send_head(self):  # noqa: D102 - overrides a documented base method
        ...
        # /how and /films/halfway are the paths the page actually links to. Checked against a file
        # rather than any path, because /films is both films.html and the films/ directory, and
        # production serves the page rather than redirecting to the directory.
        if not path.endswith("/") and not (SITE / path.lstrip("/")).is_file():
